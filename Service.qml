...
Item {
  ...
  function refreshViewSettings() {
    var entry = {}
    var pluginId = manifest && manifest.id ? String(manifest.id) : "dumidu.orbital-lock"
    ...
    viewSettings = PluginConfig.normalize(entry)
  }

  function requestPowerAction(action) {
    var requested = String(action || "")
    if (previewVisible || !sessionLock.secure || powerActionRunning) return
    if (requested !== "reboot" && requested !== "shutdown") return

    powerFailureMessage = ""
    powerActionRunning = true
    powerProcess.command = ["omarchy", "system", requested]
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        ...
        settings: root.viewSettings
        previewMode: false
        sessionSecure: sessionLock.secure
        powerActionRunning: root.powerActionRunning
        powerFailureMessage: root.powerFailureMessage
  ...
  PanelWindow {
    ...
    LockView {
      ...
      backgroundVersion: root.backgroundVersion
      fingerprintConfigured: root.fingerprintConfigured
      fingerprintAuthenticating: root.fingerprintConfigured
      authenticatingPassword: false
      failureMessage: ""
      failedAttempts: 0
      inputEnabled: false
  ...
  Process {
    ...
    onExited: function(exitCode) {
      ...
      if (exitCode !== 0) {
        root.powerFailureMessage = "Power action failed"
        powerFailureClearTimer.restart()
      }
    }
  }

  Timer {
    id: powerFailureClearTimer
    interval: 4000
    repeat: false
    onTriggered: root.powerFailureMessage = ""
  }

  Timer {
    id: idleBlankTimer
    interval: root.viewSettings.screenBlankSeconds * 1000
    repeat: false
  ...
  Connections {
    ...
    function onShellConfigChanged() { root.refreshViewSettings() }
  }

  onShellChanged: refreshViewSettings()
  onManifestChanged: refreshViewSettings()
